    ...
    <div class="col-lg-8">
        ...
        <div class="card mb-4" style="border-radius: 16px; border: 1px solid #e2e8f0;">
            <div class="card-body p-4">
                <h5 class="fw-bold mb-1" style="color: #1e293b;">Proxy Configuration</h5>
                <p class="text-muted mb-4">Configure your application to use {{ $proxyType->name }}.</p>

                @if($proxyServers->isEmpty())
                    <div class="alert" style="border-radius: 12px; background-color: #fef9c3; color: #854d0e; border: none;">
                        ...
                        <strong>No servers available yet.</strong> Please contact support.
                    </div>
                @else
                    <h6 class="fw-semibold mb-3" style="color: #475569;">Available Server Locations</h6>
                    <div class="table-responsive mb-4">
                        <table class="table table-borderless mb-0">
                            <thead>
                                <tr style="background-color: #f8fafc; border-radius: 8px;">
                                    <th style="padding: 10px 16px; color: #64748b; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Location</th>
                                    <th style="padding: 10px 16px; color: #64748b; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Proxy Host</th>
                                    <th style="padding: 10px 16px; color: #64748b; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Port</th>
                                    <th style="padding: 10px 16px; color: #64748b; font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Protocol</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($proxyServers as $server)
                                    <tr style="border-bottom: 1px solid #f1f5f9;">
                                        <td style="padding: 12px 16px; vertical-align: middle;">
                                            <span style="font-weight: 500; color: #1e293b;">{{ $server->location ?? 'Default' }}</span>
                                        </td>
                                        <td style="padding: 12px 16px; vertical-align: middle;">
                                            <code style="background-color: #f1f5f9; padding: 5px 10px; border-radius: 6px; font-size: 0.875rem; user-select: all;">{{ $server->hostname ?: $server->ip }}</code>
                                        </td>
                                        <td style="padding: 12px 16px; vertical-align: middle;">
                                            <code style="background-color: #f1f5f9; padding: 5px 10px; border-radius: 6px; font-size: 0.875rem;">{{ $server->port }}</code>
                                        </td>
                                        <td style="padding: 12px 16px; vertical-align: middle;">
                                            <span style="font-size: 0.875rem; color: #475569;">HTTP/HTTPS</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div class="alert alert-info" style="border-radius: 12px; border: none; background-color: #dbeafe; color: #1e40af;">
                        ...
                        <strong>Authentication:</strong> Use your proxy username and password from the <a href="{{ route('user.proxies.subscriptions', $proxyType->slug) }}" style="color: #1d4ed8;">My Subscriptions</a> tab.
                        All locations accept the same credentials.
                    </div>
                @endif
            </div>
        </div>
        ...
        @php
            $exServer = $proxyServers->first();
            $exHost   = $exServer->hostname ?: $exServer->ip;
            $exPort   = $exServer->port;
        @endphp
        <div class="card" style="border-radius: 16px; border: 1px solid #e2e8f0;">
            <div class="card-body p-4">
                <h5 class="fw-bold mb-1" style="color: #1e293b;">Code Examples</h5>
                <p class="text-muted mb-4">Replace <code>username</code> / <code>password</code> with your credentials. Swap the host for any location above.</p>
...
proxies = {
    'http':  'http://username:password@{{ $exHost }}:{{ $exPort }}',
    'https': 'http://username:password@{{ $exHost }}:{{ $exPort }}'
}
...
print(response.json())</code></pre>
                ...
                <div class="mb-4">
                    ...
                    <pre style="background-color: #1e293b; color: #e2e8f0; padding: 20px; border-radius: 12px; overflow-x: auto; font-size: 0.85rem; line-height: 1.6;"><code>curl -x http://{{ $exHost }}:{{ $exPort }} \
     -U username:password \
...
curl_setopt($ch, CURLOPT_PROXY, '{{ $exHost }}:{{ $exPort }}');
    ...
    <div class="col-lg-4">
            ...
            <div class="card-body p-4">
                ...
                <ul class="list-unstyled mb-0">
                    <li class="mb-3 d-flex align-items-start">
                        ...
                        <span style="font-size: 0.9rem; color: #475569;">All locations share the same username &amp; password</span>
                    </li>
                    <li class="mb-3 d-flex align-items-start">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #3b82f6; margin-right: 12px; flex-shrink: 0; margin-top: 2px;"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        <span style="font-size: 0.9rem; color: #475569;">Switch location by changing the hostname in your config</span>
                    </li>
                    <li class="mb-3 d-flex align-items-start">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #3b82f6; margin-right: 12px; flex-shrink: 0; margin-top: 2px;"><polyline points="20 6 9 17 4 12"></polyline></svg>
                    ...
                    <li class="d-flex align-items-start">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #3b82f6; margin-right: 12px; flex-shrink: 0; margin-top: 2px;"><polyline points="20 6 9 17 4 12"></polyline></svg>
                        <span style="font-size: 0.9rem; color: #475569;">Monitor bandwidth usage in the Subscriptions tab</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="card" style="border-radius: 16px; border: 1px solid #e2e8f0;">
            <div class="card-body p-4">
                ...
                <p class="text-muted mb-3" style="font-size: 0.9rem;">Check out our documentation for more examples and troubleshooting guides.</p>
