...
<div class="container-fluid">
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
        <div>
            <h3 class="fw-bold mb-1">Proxy Types</h3>
            <p class="text-muted mb-0">Manage proxy service types and categories</p>
        </div>
        <a href="{{ route('admin.proxy-types.creator') }}" class="btn btn-success" style="border-radius: 8px; padding: 10px 20px; font-weight: 500;">
                ...
                <table class="table mb-0" style="border-collapse: separate; border-spacing: 0;">
                    <thead>
                        <tr style="background-color: #f8fafc;">
                            ...
                            <th scope="col" style="padding: 1rem; color: #475569; font-weight: 600; font-size: 0.875rem;">Name</th>
                            <th scope="col" style="padding: 1rem; color: #475569; font-weight: 600; font-size: 0.875rem;">Slug</th>
                            <th scope="col" style="padding: 1rem; color: #475569; font-weight: 600; font-size: 0.875rem;">Plans</th>
                    ...
                    <tbody>
                        @forelse($proxyTypes as $type)
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                <td style="padding: 1rem;">#{{ $type->id }}</td>
                                ...
                                <td style="padding: 1rem;">
                                    <code style="background-color: #f1f5f9; padding: 4px 8px; border-radius: 4px; font-size: 0.875rem;">{{ $type->slug }}</code>
                                </td>
                                <td style="padding: 1rem;">
                                    <span class="badge bg-secondary">{{ $type->plans->count() }} plans</span>
                                </td>
                                <td style="padding: 1rem;">
                                    @if($type->is_active)
                                ...
                                <td style="padding: 1rem; text-align: right;">
                                    <a href="{{ route('admin.proxy-types.editor', $type->id) }}" class="btn btn-sm" style="padding: 6px 16px; border-radius: 6px; background-color: #eff6ff; color: #2563eb; text-decoration: none;">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.proxy-types.destroy', $type->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Are you sure you want to delete this proxy type?');">
                                        @csrf
